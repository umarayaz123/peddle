...
class BlogPost < ActiveRecord::Base
  ...
  default_scope :order => 'published_at DESC'
	...
	def tags_with_links
		#html = self.tags.split(/,/).collect {|t| "<a href=\"/blog/tag/#{t.strip}\">#{t.strip}</a>" }.join(', ')
    	html = self.tags.split(/,/).collect {|t| "#{t.strip}" }.join(', ')
  ...
  def total_comments
	...
	def user_name(skip_link=false)
		if !skip_link && BlogKit.instance.settings['link_to_user']
			return "#{CGI.escapeHTML(self.user.email)}".html_safe
      #return "<a href=\"/users/#{self.user.id}\">#{CGI.escapeHTML(self.user.email)}</a>".html_safe
		else
